        ...
        $(function () {
            ...
            function sendRawFile(file) {
                ...
                $.ajax({
                    data: data,
                    type: "POST",
                    url: "../pages/forum_upload.php", //Your own back-end uploader
                    cache: false,
                    contentType: false,
                    processData: false,
...
<body>
    ...
    <div class="container" id="container" style="padding-top: 88px">
        ...
        <a href="../forum/create" class="btn bg-smd"><i class="fas fa-plus"></i> สร้างโพสต์ใหม่!</a>
        </div>
        <table class="table table-sm table-hover bg-white" id="forumTable">
            ...
            <tbody>
                ...
                ?>                
                <tr class="table-pointer <?php if (($isThisPostMainCommentWasDeleted || $isThisPostWasHide) && (!isLogin() && getRole($_SESSION['id'], $conn) == "admin")) echo "d-none"; else if ($isThisPostMainCommentWasDeleted && (isLogin() && getRole($_SESSION['id'], $conn) == "admin")) echo "table-danger font-italic text-dark"; else if ($isThisPostWasHide) echo "table-info font-italic text-dark"; else echo "table-normal"; ?>" style="cursor: pointer;" onclick="window.location='../forum/<?php echo $IDD;?>';">
                    <td><?php if ($row['isPinned']) echo '<div class="font-weight-bold"><span class="badge red"><i class="fas fa-thumbtack"></i></span> '; else echo "<div>";?> <?php echo $title; if ($isThisPostWasHide && (isLogin() && getRole($_SESSION['id'], $conn) == "admin")) echo " <span class='badge badge-info'>โพสต์นี้ถูกซ่อน</span>"; else if ($isThisPostMainCommentWasDeleted && (isLogin() && getRole($_SESSION['id'], $conn) == "admin")) echo " <span class='badge badge-danger'>โพสต์นี้ถูกลบ</span>"; echo '</div>' . $time; ?></td>
                    <td>
        ...
        <div class="card mb-4">
            <?php if ($comment_id == 1) {
                if ($isThisPostMainCommentWasDeleted) { $_SESSION['forum_error'] = $article; header("Location: ../forum/error"); } ?>
                    ...
                    <div class="col-12 col-md-9">
                        <div class="card-text">
                            ...
                            <div style="color: #949494"><?php echo $time; ?>
                                <?php if (isLogin() && ($writer == $_SESSION['id'] || getRole($_SESSION['id'], $conn) == "admin") && !$delete) { ?>
                                    <?php if (getRole($_SESSION['id'], $conn) == "admin") {?><a href="../forum/edit-<?php echo $postID; ?>-<?php echo $row["id"]; ?>"><i class="fas fa-edit text-success"></i></a><?php } ?>
                                <a href='#' class='text-danger' onclick='
                                    swal({title: "ลบความคิดเห็นนี้หรือไม่ ?",text: "หลังจากที่ลบแล้ว จะไม่สามารถกู้คืนได้!",icon: "warning",buttons: true,dangerMode: true}).then((willDelete) => { if (willDelete) { window.location = "../pages/forum_delete.php?method=<?php if ($writer == $_SESSION["id"]) echo "user"; else if (isLogin() && getRole($_SESSION["id"], $conn) == "admin") echo "admin"; ?>&thread=<?php echo $postID; ?>&comment=<?php echo $row["id"]; ?>";}});'><i class='fas fa-trash-alt'></i></a> <?php } ?></div>
                        </div>
                    </div>
        ...
        <?php } ?>
        <?php if (isLogin()) { ?>
        <form method="POST" action="../pages/forum_reply.php?threads=<?php echo $_GET['id']; ?>"
